...
class Manager():
    ...
    def self_info_edit(self):
        ...
        if selfedit == 2 :
            ...
            connect.commit()
    def add_new_user(self) : New_User(access=self.access)
    def edit_user(self):
        if self.access == True:
...
def edit(table , valuse):
    ...
    for postx in table :
        if getusername in Item_Getter(table,valuse) :
            if interlevel == 1 :
                curs.execute(f"UPDATE {postx} SET Level = :newlevel WHERE User_Name = :interuser",{"newlevel" : "Advanced" ,"interuser" : getusername})
                connect.commit()
            if interlevel == 2 :
                curs.execute(f"UPDATE {postx} SET Level = :newlevel WHERE User_Name = :interuser",{"newlevel" : "Average" ,"interuser" : getusername})
                connect.commit()
            if interlevel == 3 :
                curs.execute(f"UPDATE {postx} SET Level = :newlevel WHERE User_Name = :interuser",{"newlevel" : "Elementary" ,"interuser" : getusername})
                connect.commit()
#-----------
...
def Print_EmployeeS_Join_Date(joinDate):
    ...
    employee = tabulate(curs.fetchall())
    print(f"Users Added In {joinDate}:\n{boss}")
    print(f"Users Added In {joinDate}:\n{assistant}")
    print(f"Users Added In {joinDate}:\n{employee}")
#-----------
...
# --------------------------------------------
Manager.all_user()
# Print_EmployeeS_Join_Date("2021-06-18")
login(input("User_Name : ") , input("Password : ")," ")